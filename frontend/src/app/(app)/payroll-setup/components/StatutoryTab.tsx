...
export default function StatutoryTab() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const [entData, confData] = await Promise.all([
          fetchJson<{ results: LegalEntity[] }>('/api/payroll-setup/legal-entities/'),
          fetchJson<{ results: StatutoryConfig[] }>('/api/payroll-setup/statutory-configs/'),
        ]);
  ...
  const contributionTypes = [
    ...
    { value: 'LWF', label: 'Labor Welfare Fund' },
