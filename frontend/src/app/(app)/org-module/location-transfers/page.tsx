...
import { locationTransfers } from '@/lib/mock/org/phase2';
...
const fields: FieldDef[] = [
  { name: 'employee', label: 'Employee', placeholder: 'e.g. Rahul Verma' },
  { name: 'from', label: 'From (location)', placeholder: 'e.g. Bengaluru HQ' },
  { name: 'to', label: 'To (location)', placeholder: 'e.g. Mumbai Office' },
...
export default function LocationTransfersPage() {
  ...
  const visible = useMemo(() => {
    ...
    return locationTransfers.filter((c) => {
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          ...
          subtitle="Moves between locations with effective dates (mock data, stub actions)."
        />
        {canManage ? (
          <button
            type="button"
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
          ...
          <span className="text-xs text-slate-500">
            Showing {visible.length} of {locationTransfers.length}
          </span>
        </div>
        {visible.length === 0 ? (
      ...
      {modalOpen ? (
        <StubModal title="Log location transfer" fields={fields} onClose={() => setModalOpen(false)} />
      ) : null}
    </div>
  );
}
