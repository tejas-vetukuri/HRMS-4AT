...
import { promotions } from '@/lib/mock/org/phase2';
...
const fields: FieldDef[] = [
  ...
  { name: 'from', label: 'From (current title)', placeholder: 'e.g. Software Engineer' },
  { name: 'to', label: 'To (new title)', placeholder: 'e.g. Senior Software Engineer' },
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          title="Promotions"
          subtitle="Effective-dated promotion history with audit trail (mock data, stub actions)."
        />
        {canManage ? (
          <button
            type="button"
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
          ...
          <span className="text-xs text-slate-500">
            Showing {visible.length} of {promotions.length}
          </span>
        </div>
        {visible.length === 0 ? (
      ...
      {modalOpen ? (
        <StubModal title="Log promotion" fields={fields} onClose={() => setModalOpen(false)} />
      ) : null}
    </div>
  );
}
