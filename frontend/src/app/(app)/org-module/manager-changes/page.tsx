...
const CHANGE_TYPE = 'manager_change' as const;
...
export default function ManagerChangesPage() {
  ...
  const managerList = useMemo<NamedEntity[]>(
    () => employees.map((e) => ({ id: e.id, name: `${fullName(e)} (${e.employee_code})` })),
    [employees],
  );
  ...
  const resolvers = useMemo(
    () => [
      {
        match: (key: string) => (key === 'manager_id' || key === 'manager' ? managerList : null),
        label: () => 'Manager',
      },
    ],
    [managerList],
  ...
  if (!managerId) {
    setFormError('Select a new manager.');
    return;
  }
  if (managerId === employeeId) {
    setFormError('An employee cannot report to themselves.');
    return;
  }
    if (!effectiveDate) {
  ...
  if (managerId) to_data.manager_id = managerId;
    const from_data: Record<string, unknown> = {};
  if (emp?.manager_id) from_data.manager_id = emp.manager_id;
    setSaving(true);
    try {
      ...
      setFormError(e instanceof OrgApiError ? e.message : 'Could not log the manager change.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          ...
          subtitle="Reporting-line changes with effective dates."
        />
        {canManage ? (
          <button
            type="button"
      ...
      {modalOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Log manager change"
        >
          <div
            className="w-full max-w-md bg-white rounded-xl border border-slate-200 shadow-lg p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-sm font-bold text-slate-900">Log manager change</h3>
            {formError ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              {field('To (new manager)', managerId, setManagerId, employees.filter((e) => e.id !== employeeId).map((e) => ({ value: e.id, label: `${fullName(e)} (${e.employee_code})` })))}
              <label className="block text-xs font-medium text-slate-600">
                Effective date
                <input
                  type="date"
            ...
            <div className="mt-5 flex gap-2 justify-end">
              ...
              >
                {saving ? 'Saving…' : 'Log manager change'}
              </button>
            </div>
          </div>
        </div>
