...
import { fullName, orgApi, OrgApiError, type NamedEntity, type OrgEmployee, type Position } from '@/lib/api/org';
import {
  orgChangesApi,
  type OrgChange,
...
const CHANGE_TYPE = 'position_change' as const;
...
export default function PositionChangesPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoadFailed(false);
    try {
      const [list, emps, poss] = await Promise.all([
  ...
  const resolvers = useMemo(
    () => [
      {
        match: (key: string) => (key === 'position_id' || key === 'position' ? positions : null),
        label: () => 'Position',
      },
    ],
    [positions],
  ...
  if (!positionId) {
    setFormError('Select a new position.');
    return;
  }
    if (!effectiveDate) {
  ...
  if (positionId) to_data.position_id = positionId;
    ...
    try {
      ...
      setFormError(e instanceof OrgApiError ? e.message : 'Could not log the position change.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          ...
          subtitle="Moves between approved positions with effective dates."
        />
        {canManage ? (
          <button
            type="button"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[200px] flex-1">
            ...
            <input
              ...
              placeholder="Search by employee, position or changed-by…"
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
          <h3 className="text-sm font-bold text-slate-900">Position Changes</h3>
      ...
      {modalOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Log position change"
        >
          <div
            className="w-full max-w-md bg-white rounded-xl border border-slate-200 shadow-lg p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-sm font-bold text-slate-900">Log position change</h3>
            {formError ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              {field('To (new position, from masters)', positionId, setPositionId, positions.map((e) => ({ value: e.id, label: e.name })))}
              <label className="block text-xs font-medium text-slate-600">
                Effective date
                <input
                  type="date"
            ...
            <div className="mt-5 flex gap-2 justify-end">
              ...
              >
                {saving ? 'Saving…' : 'Log position change'}
              </button>
            </div>
          </div>
        </div>
