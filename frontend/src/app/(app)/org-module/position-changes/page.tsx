...
import { positionChanges } from '@/lib/mock/org/phase2';
...
const fields: FieldDef[] = [
  { name: 'employee', label: 'Position', placeholder: 'e.g. POS-1012 Senior Designer' },
  { name: 'from', label: 'From (status / title)', placeholder: 'e.g. On Hold' },
  { name: 'to', label: 'To (status / title)', placeholder: 'e.g. Hiring' },
...
export default function PositionChangesPage() {
  ...
  const visible = useMemo(() => {
    ...
    return positionChanges.filter((c) => {
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          ...
          subtitle="Title / level / reports-to changes with effective dates (mock data, stub actions)."
        />
        {canManage ? (
          <button
            type="button"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[200px] flex-1">
            ...
            <input
              ...
              placeholder="Search by position, status or changed-by…"
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
          ...
          <span className="text-xs text-slate-500">
            Showing {visible.length} of {positionChanges.length}
          </span>
        </div>
        {visible.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {['Position', 'From', 'To', 'Effective date', 'Status', 'Changed by', 'Recorded (audit)'].map(
                    (h) => (
                      <th
                        key={h}
      ...
      {modalOpen ? (
        <StubModal title="Log position change" fields={fields} onClose={() => setModalOpen(false)} />
      ) : null}
    </div>
  );
}
