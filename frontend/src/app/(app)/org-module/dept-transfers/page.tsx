...
const CHANGE_TYPE = 'dept_transfer' as const;
...
export default function DeptTransfersPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoadFailed(false);
    try {
      const [list, emps, depts, bus] = await Promise.all([
  ...
  const resolvers = useMemo(
    () => [
      {
        match: (key: string) =>
          key === 'department_id' || key === 'department' ? departments : null,
        label: () => 'Department',
      },
      {
        match: (key: string) =>
          key === 'business_unit_id' || key === 'business_unit' ? businessUnits : null,
        label: () => 'Business unit',
      },
    ],
  ...
  if (!departmentId && !businessUnitId) {
    setFormError('Select a new department or business unit.');
    return;
  }
    if (!effectiveDate) {
  ...
  if (departmentId) to_data.department_id = departmentId;
  if (businessUnitId) to_data.business_unit_id = businessUnitId;
    const from_data: Record<string, unknown> = {};
  if (emp?.department_id) from_data.department_id = emp.department_id;
  if (emp?.business_unit_id) from_data.business_unit_id = emp.business_unit_id;
    setSaving(true);
    try {
    ...
    <div>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <PageHeader
          ...
          subtitle="Moves between departments with effective dates."
        />
        {canManage ? (
          <button
            type="button"
        ...
        >
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              {field('To (new department, from masters)', departmentId, setDepartmentId, departments.map((e) => ({ value: e.id, label: e.name })))}
              {field('To (new business unit, from masters)', businessUnitId, setBusinessUnitId, businessUnits.map((e) => ({ value: e.id, label: e.name })))}
              <label className="block text-xs font-medium text-slate-600">
                Effective date
                <input
                  type="date"
