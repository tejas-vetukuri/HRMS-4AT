...
export default function RequestWfhPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [fromDate, setFromDate] = useState(() => searchParams.get('date') || '');
  const [toDate, setToDate] = useState(() => searchParams.get('date') || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await attendanceApi.requestWfh({
        start_date: fromDate,
        end_date: toDate || fromDate,
        reason: reason.trim(),
      });
      setSubmitted(true);
    } catch (err) {
      setError(err instanceof AttendanceApiError ? err.message : 'Could not submit WFH request');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-full bg-gray-50 font-['Inter']">
      <div className="bg-white border-b border-gray-200 shadow-sm px-4 sm:px-8 py-6 sm:py-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-1">Request Work From Home</h1>
            <p className="text-sm text-slate-500">Submit a WFH request for manager approval</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-8 max-w-xl">
        {submitted ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm text-center">
            ...
            <p className="text-sm text-gray-600 mb-6">Your WFH request has been sent for approval.</p>
            <button
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Let your manager know why you're requesting WFH..."
                className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600"
              />
            </div>
            {error ? (
