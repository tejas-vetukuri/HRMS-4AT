...
export async function GET(req: NextRequest) {
  try {
    ...
    const resp = NextResponse.json(
      body ?? { success: false, error: { message: 'Failed to load job-families' } },
      { status: status || 502 }
    );
    ...
    console.error('[api/job-families] failed:', err);
    return NextResponse.json(
      { success: false, error: { message: 'Failed to load job-families' } },
      { status: 500 }
    );
  }
}
