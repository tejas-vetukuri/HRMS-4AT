...
export async function GET(req: NextRequest) {
  try {
    ...
    const resp = NextResponse.json(
      body ?? { success: false, error: { message: 'Failed to load cycles' } },
      { status: status || 502 }
    );
    ...
    console.error('[api/performance/cycles] failed:', err);
    return NextResponse.json(
      { success: false, error: { message: 'Failed to load cycles' } },
      { status: 500 }
    );
  }
}
