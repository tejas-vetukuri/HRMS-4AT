...
export async function GET(req: NextRequest) {
  try {
    ...
    const resp = NextResponse.json(
      body ?? { success: false, error: { message: 'Failed to load cost centers' } },
      { status: status || 502 }
    );
    ...
    console.error('[api/cost-centers] failed:', err);
    return NextResponse.json(
      { success: false, error: { message: 'Failed to load cost centers' } },
      { status: 500 }
    );
  }
}
