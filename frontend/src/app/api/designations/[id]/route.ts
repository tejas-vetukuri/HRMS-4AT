...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const { status, body, rotated, sessionExpired } = await proxyToBackend(req, `/org/designations/${id}/`, {
      method: 'PATCH',
      body: text,
    });
    if (sessionExpired || status === 401)
      return handleUnauth(NextResponse.json({ success: false, error: { message: 'Unauthorized' } }, { status: 401 }));
    return makeResp(body ?? { success: false, error: { message: 'Failed to update designation' } }, status || 502, rotated);
  } catch (err) {
    console.error('[api/designations/[id] PATCH] failed:', err);
    return NextResponse.json({ success: false, error: { message: 'Failed to update designation' } }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    console.error('[api/designations/[id] DELETE] failed:', err);
    return NextResponse.json({ success: false, error: { message: 'Failed to remove designation' } }, { status: 500 });
  }
}
