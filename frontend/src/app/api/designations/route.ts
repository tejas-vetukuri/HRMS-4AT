...
export async function GET(req: NextRequest) {
  try {
    ...
    const resp = NextResponse.json(
      body ?? { success: false, error: { message: 'Failed to load designations' } },
      { status: status || 502 }
    );
    ...
    console.error('[api/designations] failed:', err);
    return NextResponse.json(
      { success: false, error: { message: 'Failed to load designations' } },
      { status: 500 }
    );
  }
}
